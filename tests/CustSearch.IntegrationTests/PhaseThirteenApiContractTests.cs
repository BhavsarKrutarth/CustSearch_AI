...
public sealed class PhaseThirteenApiContractTests
{
    ...
    [Theory][InlineData(nameof(CamerasTrackingController.List),PermissionCatalog.Operations.CamerasView)][InlineData(nameof(CamerasTrackingController.Create),PermissionCatalog.Operations.CamerasManage)][InlineData(nameof(CamerasTrackingController.Update),PermissionCatalog.Operations.CamerasManage)][InlineData(nameof(CamerasTrackingController.Zones),PermissionCatalog.Operations.CamerasView)][InlineData(nameof(CamerasTrackingController.AddZoneVersion),PermissionCatalog.Operations.CamerasManage)][InlineData(nameof(CamerasTrackingController.Tracks),PermissionCatalog.Operations.CamerasView)][InlineData(nameof(CamerasTrackingController.Associate),PermissionCatalog.Operations.CamerasManage)]public void TenantEndpointsRequireExactPermission(string method,string permission){var attribute=typeof(CamerasTrackingController).GetMethod(method)!.GetCustomAttribute<HasPermissionAttribute>();Assert.Equal(AuthorizationPolicyNames.ForPermission(permission),attribute?.Policy);}
