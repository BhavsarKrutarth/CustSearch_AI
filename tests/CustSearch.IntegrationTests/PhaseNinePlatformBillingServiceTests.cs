...
public sealed class PhaseNinePlatformBillingServiceTests
{
    ...
    public async Task StaffQuotaRejectsPlanBelowAuthoritativeActiveStaffUsage()
    {
        ...
        var exception = await Assert.ThrowsAsync<PlatformBusinessRuleException>(() =>
            fixture.CreateService().CreateSubscriptionAsync(fixture.TenantA.Id, new(constrained.Id, "Monthly", Now, false)));
    ...
    public async Task ConflictingDuplicatePaymentReferenceIsRejected()
    {
        ...
        await service.CreateSubscriptionAsync(fixture.TenantA.Id, new(plan.Id, "Monthly", Now, false));
        var invoice = await service.GenerateInvoiceAsync(fixture.TenantA.Id, new(0m, 0m, null));
        await service.RecordPaymentAsync(new(invoice.Id, "UPI", 50m, "INR", null, "txn-conflict", Now, PlatformPaymentStatus.Successful));

        var exception = await Assert.ThrowsAsync<PlatformBusinessRuleException>(() =>
            service.RecordPaymentAsync(new(invoice.Id, "UPI", 40m, "INR", null, "txn-conflict", Now, PlatformPaymentStatus.Successful)));
    ...
    private sealed class Fixture : IAsyncDisposable
    {
        ...
        private Fixture(SqliteConnection connection, CustSearchDbContext db, Tenant tenantA, Tenant tenantB)
        {
        ...
        public static async Task<Fixture> CreateAsync()
        {
            ...
            return new Fixture(connection, db, tenantA, tenantB);
        }

        public async Task<SubscriptionPlan> AddPlanAsync(string code, decimal monthlyPrice, int trialDays, int maxStores, int maxUsers, int maxStaff, int maxCameras)
        {
            var plan = SubscriptionPlan.CreatePlatform(code, code, $"{code} plan", monthlyPrice, monthlyPrice * 10, "INR", trialDays, maxStores, maxUsers, maxStaff, maxCameras, null, null, null, 0, Now);
            Db.SubscriptionPlans.Add(plan);
            await Db.SaveChangesAsync();
            return plan;
        }

        public async Task AddStaffAsync(Tenant tenant, string code)
        {
            var user = UserAccount.CreateTenant(tenant.Id, $"user-{code}", $"{code}@phase9.test", code, "hash", Now);
            Db.UserAccounts.Add(user);
            await Db.SaveChangesAsync();
            Db.StaffProfiles.Add(StaffProfile.Create(tenant.Id, user.Id, code, "Staff", code, null, Now));
            await Db.SaveChangesAsync();
        }

        public void UsePlatformAdmin()
        {
            currentUser.IsPlatformAdminValue = true;
            currentUser.TenantIdValue = null;
        }

        public void UseTenant(Tenant tenant)
        {
            currentUser.IsPlatformAdminValue = false;
    ...
    {
        public bool IsPlatformAdminValue { get; set; }
        public long? TenantIdValue { get; set; }
        public bool IsAuthenticated => true;
        public long UserId => 1;
        public long? TenantId => TenantIdValue;
        public bool IsPlatformAdmin => IsPlatformAdminValue;
        public string SecurityStamp => "phase9-test";
    ...
    {
        private readonly DateTimeOffset current = new(utc);
