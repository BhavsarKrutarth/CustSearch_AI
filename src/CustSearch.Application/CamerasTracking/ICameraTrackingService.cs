...
public interface ICameraTrackingService
{
    ...
    Task<CameraQuotaView> GetCameraQuotaAsync(CancellationToken ct=default);
...
public sealed record CameraQuotaView(int MaxCameras,int ConfiguredCameras,int ActiveCameras,int AvailableCameras,bool CanAddActiveCamera);
