...
{
    public void Configure(EntityTypeBuilder<CameraOperationalEvent>b){b.ToTable("CameraOperationalEvents","dbo",t=>t.HasCheckConstraint("CK_CameraEvents_Status","[Status] BETWEEN 1 AND 4"));b.HasKey(x=>x.Id);b.Property(x=>x.Id).ValueGeneratedOnAdd();b.Property(x=>x.ServiceId).HasMaxLength(100).IsRequired();b.Property(x=>x.EventId).HasMaxLength(150).IsRequired();b.Property(x=>x.IdempotencyKey).HasMaxLength(150).IsRequired();b.Property(x=>x.EventType).HasMaxLength(100).IsRequired();b.Property(x=>x.PayloadHash).HasMaxLength(64).IsRequired();b.Property(x=>x.CorrelationId).HasMaxLength(64).IsRequired();b.Property(x=>x.Status).HasConversion<byte>();b.Property(x=>x.OccurredUtc).HasPrecision(7);b.Property(x=>x.ReceivedUtc).HasPrecision(7);b.HasIndex(x=>new{x.ServiceId,x.EventId}).IsUnique();b.HasIndex(x=>new{x.ServiceId,x.IdempotencyKey}).IsUnique();b.HasIndex(x=>new{x.TenantId,x.StoreId,x.ReceivedUtc});}
}
internal sealed class CameraUserPreviewGrantConfiguration:IEntityTypeConfiguration<CameraUserPreviewGrant>
{
    public void Configure(EntityTypeBuilder<CameraUserPreviewGrant>b){b.ToTable("CameraUserPreviewGrants","dbo",t=>t.HasCheckConstraint("CK_CameraUserPreviewGrants_Access","[CanViewLive]=1 OR ([CanViewTracking]=0 AND [CanControl]=0)"));b.HasKey(x=>x.Id);b.Property(x=>x.Id).ValueGeneratedOnAdd();b.Property(x=>x.CreatedUtc).HasPrecision(7);b.Property(x=>x.UpdatedUtc).HasPrecision(7);b.Property(x=>x.ValidUntilUtc).HasPrecision(7);b.Property(x=>x.RowVersion).IsRowVersion();b.HasIndex(x=>new{x.TenantId,x.CameraId,x.UserId}).IsUnique();b.HasIndex(x=>new{x.TenantId,x.UserId,x.IsActive,x.ValidUntilUtc});}
}
internal sealed class CameraPreviewSessionConfiguration:IEntityTypeConfiguration<CameraPreviewSession>
{
    public void Configure(EntityTypeBuilder<CameraPreviewSession>b){b.ToTable("CameraPreviewSessions","dbo",t=>{t.HasCheckConstraint("CK_CameraPreviewSessions_Status","[Status] BETWEEN 1 AND 3");t.HasCheckConstraint("CK_CameraPreviewSessions_Expiry","[ExpiresUtc]>[CreatedUtc]");});b.HasKey(x=>x.Id);b.Property(x=>x.Id).ValueGeneratedNever();b.Property(x=>x.Status).HasConversion<byte>();b.Property(x=>x.CreatedUtc).HasPrecision(7);b.Property(x=>x.ExpiresUtc).HasPrecision(7);b.Property(x=>x.LastAccessedUtc).HasPrecision(7);b.Property(x=>x.EndedUtc).HasPrecision(7);b.HasIndex(x=>new{x.TenantId,x.UserId,x.Status,x.ExpiresUtc});b.HasIndex(x=>new{x.CameraId,x.Status,x.ExpiresUtc});}
}
