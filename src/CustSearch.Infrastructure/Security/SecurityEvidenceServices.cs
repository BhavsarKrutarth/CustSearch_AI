...
public sealed class LocalEncryptedSecurityEvidenceStore(IOptions<SecurityEvidenceOptions>options):ISecurityEvidenceStore
{
    ...
    public async Task SaveEncryptedAsync(string objectKey,ReadOnlyMemory<byte>content,CancellationToken ct=default)
    {if(content.IsEmpty)throw new SecurityException("Evidence content is empty.",SecurityFailureKind.Validation);var path=Resolve(objectKey);Directory.CreateDirectory(Path.GetDirectoryName(path)!);var nonce=RandomNumberGenerator.GetBytes(12);var tag=new byte[16];var ciphertext=new byte[content.Length];using(var aes=new AesGcm(key,16))aes.Encrypt(nonce,content.Span,ciphertext,tag);var envelope=new byte[28+ciphertext.Length];nonce.CopyTo(envelope,0);tag.CopyTo(envelope,12);ciphertext.CopyTo(envelope,28);var temporary=path+"."+Guid.NewGuid().ToString("N",CultureInfo.InvariantCulture)+".tmp";try{await File.WriteAllBytesAsync(temporary,envelope,ct).ConfigureAwait(false);File.Move(temporary,path,false);}finally{if(File.Exists(temporary))File.Delete(temporary);}}
    ...
    public Task<bool>ExistsAsync(string objectKey,CancellationToken ct=default){ct.ThrowIfCancellationRequested();return Task.FromResult(File.Exists(Resolve(objectKey)));}
