    ...
    public async Task<AlertView> CreateAsync(CreateAlertCommand command,TenantAuditContext audit,CancellationToken cancellationToken=default)
    {
        ...
        return await deduplication.ExecuteAsync(tenantId,key,async()=>
        {
            ...
            try
            {
                ...
                await QueueEventAsync(alert,"alert.created",audit,now,cancellationToken).ConfigureAwait(false);db.AuditLogs.Add(AuditLog.Record(tenantId,alert.StoreId,audit.ActorUserId,"User","AlertCreated","Alert",alert.Id.ToString(),null,JsonSerializer.Serialize(Map(alert),JsonOptions),audit.IpAddress,audit.UserAgent,audit.CorrelationId,now));await db.SaveChangesAsync(cancellationToken).ConfigureAwait(false);await transaction.CommitAsync(cancellationToken).ConfigureAwait(false);return Map(alert);
            }
