...
export interface StorageUsage{tenantId:number;quotaBytes:number;usedBytes:number;availableBytes:number;snapshotBytes:number;motionClipBytes:number;securityEvidenceBytes:number;otherBytes:number;usagePercent:number;pressureLevel:string;lastCalculatedUtc:string;lastCleanupUtc:string|null;lastReconciledUtc:string|null;}
