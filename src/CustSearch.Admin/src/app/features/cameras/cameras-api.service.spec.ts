...
describe('CamerasApiService',()=>{
  ...
  it('saves motion settings without a client TenantId',()=>{service.saveMotionSettings(31,true).subscribe();const request=http.expectOne('/api/tenant/cameras/31/motion-settings');expect(request.request.method).toBe('PUT');expect(request.request.body).toEqual({enabled:true});expect(request.request.body.tenantId).toBeUndefined();request.flush({cameraId:31,motionRulesEnabled:true,useDetectionZone:false});});
  it('saves the optional detection zone setting without a client TenantId',()=>{service.saveDetectionZoneSetting(31,true).subscribe();const request=http.expectOne('/api/tenant/cameras/31/detection-zone-setting');expect(request.request.method).toBe('PUT');expect(request.request.body).toEqual({enabled:true});expect(request.request.body.tenantId).toBeUndefined();request.flush({cameraId:31,motionRulesEnabled:true,useDetectionZone:true});});
