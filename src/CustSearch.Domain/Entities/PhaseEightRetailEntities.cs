...
public sealed class RetailInvoice
{
    ...
    public void SetCalculatedTotals(decimal subtotal,decimal discount,decimal tax,decimal grandTotal,DateTime utcNow)
    {
        ...
        ArgumentOutOfRangeException.ThrowIfGreaterThan(discount,subtotal,nameof(discount));var expected=Money(subtotal-discount+tax,"expected");if(expected!=grandTotal)throw new ArgumentException("GrandTotal must equal Subtotal - DiscountAmount + TaxAmount.",nameof(grandTotal));
    ...
    {
        paidAmount=Money(paidAmount,nameof(paidAmount));ArgumentOutOfRangeException.ThrowIfGreaterThan(paidAmount,GrandTotal,nameof(paidAmount));PaidAmount=paidAmount;BalanceAmount=Money(GrandTotal-PaidAmount,nameof(BalanceAmount));
