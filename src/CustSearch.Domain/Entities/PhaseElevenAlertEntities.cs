...
public sealed class RealtimeEvent
{
    ...
    {ArgumentOutOfRangeException.ThrowIfNegativeOrZero(tenantId);if(storeId.HasValue)ArgumentOutOfRangeException.ThrowIfNegativeOrZero(storeId.Value);ArgumentOutOfRangeException.ThrowIfNegativeOrZero(alertId);ArgumentOutOfRangeException.ThrowIfLessThan(contractVersion,1);TenantId=tenantId;StoreId=storeId;AlertId=alertId;EventName=Required(eventName,100);ContractVersion=contractVersion;CorrelationId=Required(correlationId,64);DeduplicationKey=Required(deduplicationKey,200);OccurredUtc=Utc(occurredUtc);PayloadJson="{}";}
...
public sealed class NotificationOutboxMessage
{
    ...
    {ArgumentOutOfRangeException.ThrowIfNegativeOrZero(tenantId);if(storeId.HasValue)ArgumentOutOfRangeException.ThrowIfNegativeOrZero(storeId.Value);ArgumentOutOfRangeException.ThrowIfNegativeOrZero(alertId);ArgumentOutOfRangeException.ThrowIfNegativeOrZero(realtimeEventId);ArgumentOutOfRangeException.ThrowIfLessThan(contractVersion,1);TenantId=tenantId;StoreId=storeId;AlertId=alertId;RealtimeEventId=realtimeEventId;Channel=Required(channel,30);EventType=Required(eventType,100);ContractVersion=contractVersion;PayloadJson=Required(payloadJson,16000);CorrelationId=Required(correlationId,64);IdempotencyKey=Required(idempotencyKey,200);CreatedUtc=Utc(createdUtc);NextAttemptUtc=CreatedUtc;Status=NotificationOutboxStatus.Pending;}
    ...
    public void StartAttempt(DateTime utcNow,TimeSpan processingLease){if(Status is NotificationOutboxStatus.Delivered or NotificationOutboxStatus.DeadLetter)throw new InvalidOperationException("A terminal outbox message cannot be processed.");utcNow=Utc(utcNow);ArgumentOutOfRangeException.ThrowIfLessThanOrEqual(processingLease,TimeSpan.Zero);if(utcNow<NextAttemptUtc)throw new InvalidOperationException("The outbox message is not due.");AttemptCount++;Status=NotificationOutboxStatus.Processing;NextAttemptUtc=utcNow+processingLease;LastError=null;}
    ...
    public void MarkFailed(string error,DateTime utcNow,int maxAttempts,TimeSpan retryDelay){if(Status!=NotificationOutboxStatus.Processing)throw new InvalidOperationException("Only a processing message can fail.");ArgumentOutOfRangeException.ThrowIfLessThan(maxAttempts,1);utcNow=Utc(utcNow);LastError=Required(error,2000);if(AttemptCount>=maxAttempts){Status=NotificationOutboxStatus.DeadLetter;ProcessedUtc=utcNow;return;}Status=AttemptCount==1?NotificationOutboxStatus.Failed:NotificationOutboxStatus.Retrying;NextAttemptUtc=utcNow+retryDelay;}
    public void RetryManually(DateTime utcNow){if(Status is not(NotificationOutboxStatus.DeadLetter or NotificationOutboxStatus.Failed or NotificationOutboxStatus.Retrying))throw new InvalidOperationException("Only failed, retrying or dead-letter notifications can be retried manually.");Status=NotificationOutboxStatus.Retrying;AttemptCount=0;NextAttemptUtc=Utc(utcNow);ProcessedUtc=null;LastError=null;}
    private static string Required(string value,int max){ArgumentException.ThrowIfNullOrWhiteSpace(value);var v=value.Trim();return v.Length<=max?v:throw new ArgumentOutOfRangeException(nameof(value));}
    private static DateTime Utc(DateTime value)=>value.Kind==DateTimeKind.Utc?value:throw new ArgumentException("Timestamp must be UTC.",nameof(value));
}
