...
public sealed partial class ExportJobsHostedService(IServiceScopeFactory scopes,IOptions<ExportWorkerOptions>options,IOptions<OperationalPlatformOptions>operationalOptions,ILogger<ExportJobsHostedService>logger):BackgroundService
{
    private readonly ExportWorkerOptions settings=options.Value;
    protected override async Task ExecuteAsync(CancellationToken stoppingToken){while(!stoppingToken.IsCancellationRequested){try{await using var scope=scopes.CreateAsyncScope();var gate=scope.ServiceProvider.GetRequiredService<IWorkerRuntimeGate>();var lease=await gate.TryAcquireAsync("exports",Environment.MachineName,TimeSpan.FromSeconds(operationalOptions.Value.LeaseSeconds),stoppingToken).ConfigureAwait(false);if(lease is not null){try{var processor=scope.ServiceProvider.GetRequiredService<IExportJobProcessor>();var result=await processor.ProcessDueAsync(settings.BatchSize,stoppingToken).ConfigureAwait(false);var expired=await processor.ExpireDueAsync(settings.BatchSize*5,stoppingToken).ConfigureAwait(false);if(result.Claimed>0||expired>0)LogProcessed(logger,result.Claimed,result.Completed,result.Failed,expired);}finally{await gate.ReleaseAsync(lease,stoppingToken).ConfigureAwait(false);}}}catch(OperationCanceledException)when(stoppingToken.IsCancellationRequested){break;}catch(Exception exception){LogCycleFailure(logger,exception);}await Task.Delay(TimeSpan.FromSeconds(settings.PollIntervalSeconds),stoppingToken).ConfigureAwait(false);}}
