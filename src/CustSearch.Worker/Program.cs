...
try
{
    ...
    builder.Services.AddOptions<EvidenceRetentionOptions>().Bind(builder.Configuration.GetSection(EvidenceRetentionOptions.SectionName)).Validate(x=>x.IsValid(),"EvidenceRetention settings are invalid.").ValidateOnStart();
...
builder.Services.AddHostedService<EvidenceRetentionHostedService>();
