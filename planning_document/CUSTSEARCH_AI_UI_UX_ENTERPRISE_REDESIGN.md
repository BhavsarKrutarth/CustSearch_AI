...
No route guard, role list, permission name, tenant request model, API endpoint, camera preview session, SignalR stream, or login response contract was renamed or weakened. Tenant dashboards consume the existing tenant summary endpoint; platform metrics consume the existing platform dashboard endpoint. No TenantId was introduced into browser request models.
