...
| 2/5/17 | auth API/application/infrastructure | added self-service change password and tenant-admin in-scope password reset with one-way hashing, security-stamp rotation, all-session revocation and secret-free audit events | provide supported recovery instead of hash decoding/manual SQL | pinned .NET 8 build; unit 104/104; integration 230/230 |
| 2/5/17 | Angular auth and tenant-user UI | added responsive account-security route/link, self change form, administrator reset form, confirmation/policy validation and post-change sign-in message | make password lifecycle executable from UI for Platform and Tenant identities | lint PASS; Angular 82/82; production build PASS |
| 17 | `docs/ADMIN_USER_DATABASE_LOGIN_AND_PASSWORD_FLOW.md` | documented live UAT IDs, auth tables, platform/tenant flows, safe password rotation and read-only diagnostics without secret values | provide a project-wise database/login support reference | live SQL inventory plus source traceability verified |
