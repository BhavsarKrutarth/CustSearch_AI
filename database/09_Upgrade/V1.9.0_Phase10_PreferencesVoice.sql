/*
 ...
 Scope: factual customer preference signals, derived scores, verified-household preference tags,
        versioned recalculation weights and confirmation-controlled staff voice sessions.
 Rules: SQL Server 2022, idempotent, no EF migrations, UTC timestamps.
 Privacy: VisitParty/co-visit is NEVER used to create Household preference truth.
 Voice: Phase 5 StoreVoiceCommandSettings + aliases remain the trigger master; Phase 10 adds a one-to-one runtime extension.
*/
USE [CustSearch_AI];
GO
SET XACT_ABORT ON;
...
-- Derived scores are intentionally stored in a separate table.
-- ============================================================
...
BEGIN
    ...
    (
        ...
        SignalScore DECIMAL(6,2) NULL,
        Source TINYINT NOT NULL,
        Confidence DECIMAL(6,2) NULL,
        FirstObservedUtc DATETIME2(7) NOT NULL,
        LastObservedUtc DATETIME2(7) NOT NULL,
...
END;
GO
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.CustomerPreferenceSignals') AND name=N'UX_CustomerPreferenceSignals_Tenant_Id')
    CREATE UNIQUE INDEX UX_CustomerPreferenceSignals_Tenant_Id ON dbo.CustomerPreferenceSignals(TenantId,Id);
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.CustomerPreferenceSignals') AND name=N'IX_CustomerPreferenceSignals_Tenant_Customer_Active')
    CREATE INDEX IX_CustomerPreferenceSignals_Tenant_Customer_Active ON dbo.CustomerPreferenceSignals(TenantId,CustomerId,IsActive,LastObservedUtc DESC);
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.CustomerPreferenceSignals') AND name=N'IX_CustomerPreferenceSignals_Tenant_Store_Customer')
    CREATE INDEX IX_CustomerPreferenceSignals_Tenant_Store_Customer ON dbo.CustomerPreferenceSignals(TenantId,StoreId,CustomerId);
GO

-- ============================================================
-- 10F — VERSIONED RECALCULATION WEIGHTS
-- ============================================================
...
BEGIN
    ...
    (
        ...
        ManualStaffWeight DECIMAL(6,3) NOT NULL,
        PurchaseWeight DECIMAL(6,3) NOT NULL,
        CategoryInteractionWeight DECIMAL(6,3) NOT NULL,
        VoiceConfirmedWeight DECIMAL(6,3) NOT NULL,
...
END;
GO
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.PreferenceWeightVersions') AND name=N'UX_PreferenceWeightVersions_Tenant_Code')
    CREATE UNIQUE INDEX UX_PreferenceWeightVersions_Tenant_Code ON dbo.PreferenceWeightVersions(TenantId,VersionCode);
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.PreferenceWeightVersions') AND name=N'UX_PreferenceWeightVersions_Tenant_Id')
    CREATE UNIQUE INDEX UX_PreferenceWeightVersions_Tenant_Id ON dbo.PreferenceWeightVersions(TenantId,Id);
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.PreferenceWeightVersions') AND name=N'UX_PreferenceWeightVersions_OneActive')
    CREATE UNIQUE INDEX UX_PreferenceWeightVersions_OneActive ON dbo.PreferenceWeightVersions(TenantId) WHERE IsActive=1;
GO
...
BEGIN
    ...
    (
        ...
        WeightVersionId BIGINT NOT NULL,
        CalculatedUtc DATETIME2(7) NOT NULL,
...
END;
GO
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.CustomerPreferenceScores') AND name=N'IX_CustomerPreferenceScores_Tenant_Customer_Score')
    CREATE INDEX IX_CustomerPreferenceScores_Tenant_Customer_Score ON dbo.CustomerPreferenceScores(TenantId,CustomerId,Score DESC);
GO

-- ============================================================
-- 10B — EXPLICIT SHARED HOUSEHOLD TAGS
-- Household member scores are aggregated ONLY from verified active HouseholdMembers.
-- No VisitParty foreign key or inferred-family source is permitted here.
-- ============================================================
...
END;
GO
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.HouseholdPreferenceTags') AND name=N'IX_HouseholdPreferenceTags_Tenant_Household_Active')
    CREATE INDEX IX_HouseholdPreferenceTags_Tenant_Household_Active ON dbo.HouseholdPreferenceTags(TenantId,HouseholdId,IsActive,CreatedUtc DESC);
GO

-- ============================================================
-- 10D — PHASE 5 VOICE MASTER EXTENSION
-- TriggerKeyword/Aliases remain in StoreVoiceCommandSettings/StoreVoiceCommandAliases.
-- ============================================================
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.StoreVoiceCommandSettings') AND name=N'UX_StoreVoiceCommandSettings_Tenant_Store')
    CREATE UNIQUE INDEX UX_StoreVoiceCommandSettings_Tenant_Store ON dbo.StoreVoiceCommandSettings(TenantId,StoreId);
GO
...
BEGIN
    ...
    (
        ...
        TenantId BIGINT NOT NULL,
        LanguageCode NVARCHAR(20) NOT NULL CONSTRAINT DF_StoreVoiceRuntime_Language DEFAULT(N'en-IN'),
        RequireConfirmation BIT NOT NULL CONSTRAINT DF_StoreVoiceRuntime_Confirmation DEFAULT(1),
        ListeningTimeoutSeconds INT NOT NULL CONSTRAINT DF_StoreVoiceRuntime_Timeout DEFAULT(15),
...
END;
GO
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.StoreVoiceCommandRuntimeSettings') AND name=N'UX_StoreVoiceRuntime_Tenant_Store')
    CREATE UNIQUE INDEX UX_StoreVoiceRuntime_Tenant_Store ON dbo.StoreVoiceCommandRuntimeSettings(TenantId,StoreId);
GO

-- ============================================================
-- 10E — CONFIRMATION-CONTROLLED VOICE COMMAND SESSIONS
-- Status: 1 Listening, 2 PendingConfirmation, 3 Confirmed, 4 Rejected, 5 Expired.
-- ============================================================
...
BEGIN
    ...
    (
        ...
        CustomerId BIGINT NOT NULL,
        MatchedTrigger NVARCHAR(100) NOT NULL,
        RecognizedText NVARCHAR(250) NULL,
        RecognitionConfidence DECIMAL(6,2) NULL,
        ProposedPreferenceType TINYINT NULL,
        ProposedReferenceId BIGINT NULL,
...
END;
GO
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.VoiceCommandSessions') AND name=N'IX_VoiceCommandSessions_Tenant_Store_Customer_Status')
    CREATE INDEX IX_VoiceCommandSessions_Tenant_Store_Customer_Status ON dbo.VoiceCommandSessions(TenantId,StoreId,CustomerId,Status,CreatedUtc DESC);
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.VoiceCommandSessions') AND name=N'IX_VoiceCommandSessions_Tenant_Staff_Created')
    CREATE INDEX IX_VoiceCommandSessions_Tenant_Staff_Created ON dbo.VoiceCommandSessions(TenantId,StaffUserId,CreatedUtc DESC);
GO

-- ============================================================
-- 10G — ENSURE STABLE EXISTING PERMISSIONS
-- ============================================================
...
-- ============================================================
-- READ PROCEDURES — tenant/store predicates are applied before returning data.
-- ============================================================
CREATE OR ALTER PROCEDURE dbo.CustomerPreference_Get
    @TenantId BIGINT,
    @CustomerId BIGINT,
...
BEGIN
    SET NOCOUNT ON;
    IF NOT EXISTS(SELECT 1 FROM dbo.Customers c WHERE c.TenantId=@TenantId AND c.Id=@CustomerId AND
       (@AllowedStoreIdsCsv IS NULL OR EXISTS(SELECT 1 FROM dbo.CustomerStoreAssignments csa JOIN STRING_SPLIT(@AllowedStoreIdsCsv,N',') s ON TRY_CONVERT(BIGINT,s.value)=csa.StoreId WHERE csa.TenantId=@TenantId AND csa.CustomerId=c.Id))) RETURN;
    SELECT Id,StoreId,CustomerId,PreferenceType,ReferenceId,Value,SignalScore,Source,Confidence,FirstObservedUtc,LastObservedUtc,IsActive,Reason
    FROM dbo.CustomerPreferenceSignals WHERE TenantId=@TenantId AND CustomerId=@CustomerId ORDER BY LastObservedUtc DESC,Id DESC;
    SELECT Id,CustomerId,PreferenceType,ReferenceId,Value,Score,WeightVersionId,CalculatedUtc
    FROM dbo.CustomerPreferenceScores WHERE TenantId=@TenantId AND CustomerId=@CustomerId ORDER BY Score DESC,Id DESC;
END;
GO

CREATE OR ALTER PROCEDURE dbo.HouseholdPreference_Get
    @TenantId BIGINT,
    @HouseholdId BIGINT,
    @AllowedStoreIdsCsv NVARCHAR(MAX)=NULL
AS
BEGIN
    ...
    IF @AllowedStoreIdsCsv IS NOT NULL AND NOT EXISTS(
        ...
        WHERE hm.TenantId=@TenantId AND hm.HouseholdId=@HouseholdId AND hm.IsActive=1 AND hm.IsVerified=1) RETURN;
    SELECT hm.CustomerId,c.FirstName,c.LastName,hm.RelationshipType,hm.RelationshipSource,hm.VerifiedUtc
    FROM dbo.HouseholdMembers hm JOIN dbo.Customers c ON c.TenantId=hm.TenantId AND c.Id=hm.CustomerId
    WHERE hm.TenantId=@TenantId AND hm.HouseholdId=@HouseholdId AND hm.IsActive=1 AND hm.IsVerified=1
    ...
    SELECT s.CustomerId,s.PreferenceType,s.ReferenceId,s.Value,s.Score,s.WeightVersionId,s.CalculatedUtc
    FROM dbo.CustomerPreferenceScores s JOIN dbo.HouseholdMembers hm ON hm.TenantId=s.TenantId AND hm.CustomerId=s.CustomerId AND hm.HouseholdId=@HouseholdId AND hm.IsActive=1 AND hm.IsVerified=1
    WHERE s.TenantId=@TenantId
      AND (@AllowedStoreIdsCsv IS NULL OR EXISTS(SELECT 1 FROM dbo.CustomerStoreAssignments csa JOIN STRING_SPLIT(@AllowedStoreIdsCsv,N',') x ON TRY_CONVERT(BIGINT,x.value)=csa.StoreId WHERE csa.TenantId=@TenantId AND csa.CustomerId=s.CustomerId));
    SELECT Id,PreferenceType,ReferenceId,Value,Source,Reason,CreatedUtc FROM dbo.HouseholdPreferenceTags
    WHERE TenantId=@TenantId AND HouseholdId=@HouseholdId AND IsActive=1 ORDER BY CreatedUtc DESC;
END;
GO
...
BEGIN
    SET NOCOUNT ON;
    SELECT TOP(1) Id,VersionCode,ManualStaffWeight,PurchaseWeight,CategoryInteractionWeight,VoiceConfirmedWeight,IsActive,CreatedUtc
    FROM dbo.PreferenceWeightVersions WHERE TenantId=@TenantId AND IsActive=1 ORDER BY Id DESC;
END;
GO

CREATE OR ALTER PROCEDURE dbo.PreferenceAudit_Search
    @TenantId BIGINT,
    @AllowedStoreIdsCsv NVARCHAR(MAX)=NULL,
    @PageNumber INT=1,
    @PageSize INT=50
...
BEGIN
    ...
    SELECT Id,StoreId,UserId,Action,EntityType,EntityId,BeforeJson,AfterJson,CorrelationId,CreatedUtc,COUNT_BIG(1) OVER() TotalCount
    FROM dbo.AuditLogs
    WHERE TenantId=@TenantId
      AND (Action LIKE N'CustomerPreference%' OR Action LIKE N'HouseholdPreference%' OR Action LIKE N'PreferenceWeight%' OR Action LIKE N'VoiceCommand%' OR Action LIKE N'StoreVoice%')
...
IF NOT EXISTS(SELECT 1 FROM dbo.DatabaseVersions WHERE VersionNumber=N'V1.9.0')
    INSERT INTO dbo.DatabaseVersions(VersionNumber,Description,AppliedUtc,AppliedBy) VALUES(N'V1.9.0',N'Phase 10 preferences, verified-household aggregation, dynamic voice confirmation and versioned recalculation',SYSUTCDATETIME(),SUSER_SNAME());
GO
IF (SELECT COUNT(*) FROM dbo.DatabaseVersions WHERE VersionNumber=N'V1.9.0')<>1
    THROW 52090,'V1.9.0 DatabaseVersions row must exist exactly once.',1;
GO
