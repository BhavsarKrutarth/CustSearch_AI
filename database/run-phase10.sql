/*
 CustSearch AI — Phase 10 standalone/manual installer
 Version: V1.9.0
 Run in SSMS/Azure Data Studio against an existing Phase 9 V1.8.0 CustSearch_AI database.
 Safe to run repeatedly. No PowerShell and no sqlcmd :r include is required.

 Phase 10 objects:
  10A CustomerPreferenceSignals (factual signals)
  10B HouseholdPreferenceTags (explicit shared tags; verified HouseholdMembers only for aggregation)
  10D StoreVoiceCommandRuntimeSettings (one-to-one extension of Phase 5 trigger/alias master)
  10E VoiceCommandSessions (confirmation/rejection lifecycle)
  10F PreferenceWeightVersions + CustomerPreferenceScores (versioned deterministic recalculation)
  10G preference/voice permissions and tenant-safe read procedures

 Privacy: VisitParty/co-visit is never a Household/family preference source.
*/
USE [CustSearch_AI];
GO
SET XACT_ABORT ON;
...
IF OBJECT_ID(N'dbo.DatabaseVersions',N'U') IS NULL OR NOT EXISTS(SELECT 1 FROM dbo.DatabaseVersions WHERE VersionNumber=N'V1.8.0')
    THROW 52100,'Phase 10 requires Phase 9 V1.8.0.',1;
GO
IF OBJECT_ID(N'dbo.Customers',N'U') IS NULL OR OBJECT_ID(N'dbo.Households',N'U') IS NULL OR OBJECT_ID(N'dbo.HouseholdMembers',N'U') IS NULL OR OBJECT_ID(N'dbo.StoreVoiceCommandSettings',N'U') IS NULL
    THROW 52101,'Required Phase 5-9 baseline objects are missing.',1;
GO

-- 10A: factual customer signals; never overwrite derived-score history in this table.
...
BEGIN
 CREATE TABLE dbo.CustomerPreferenceSignals(
  ...
  TenantId BIGINT NOT NULL,StoreId BIGINT NULL,CustomerId BIGINT NOT NULL,PreferenceType TINYINT NOT NULL,ReferenceId BIGINT NULL,Value NVARCHAR(200) NULL,
  SignalScore DECIMAL(6,2) NULL,Source TINYINT NOT NULL,Confidence DECIMAL(6,2) NULL,FirstObservedUtc DATETIME2(7) NOT NULL,LastObservedUtc DATETIME2(7) NOT NULL,
  IsActive BIT NOT NULL CONSTRAINT DF_CustomerPreferenceSignals_IsActive DEFAULT(1),CreatedByUserId BIGINT NULL,Reason NVARCHAR(500) NULL,CreatedUtc DATETIME2(7) NOT NULL,UpdatedUtc DATETIME2(7) NOT NULL,
  ...
  CONSTRAINT CK_CustomerPreferenceSignals_Type CHECK(PreferenceType BETWEEN 1 AND 5),CONSTRAINT CK_CustomerPreferenceSignals_Source CHECK(Source BETWEEN 1 AND 4),
  CONSTRAINT CK_CustomerPreferenceSignals_Identity CHECK(ReferenceId IS NOT NULL OR NULLIF(LTRIM(RTRIM(Value)),N'') IS NOT NULL),
  CONSTRAINT CK_CustomerPreferenceSignals_Score CHECK(SignalScore IS NULL OR SignalScore BETWEEN 0 AND 100),CONSTRAINT CK_CustomerPreferenceSignals_Confidence CHECK(Confidence IS NULL OR Confidence BETWEEN 0 AND 100),
...
-- 10F: active version is unique per tenant so identical signals + version reproduce the same score.
IF OBJECT_ID(N'dbo.PreferenceWeightVersions',N'U') IS NULL
BEGIN
 CREATE TABLE dbo.PreferenceWeightVersions(
  Id BIGINT IDENTITY(1,1) NOT NULL CONSTRAINT PK_PreferenceWeightVersions PRIMARY KEY,TenantId BIGINT NOT NULL,VersionCode NVARCHAR(50) NOT NULL,
  ManualStaffWeight DECIMAL(6,3) NOT NULL,PurchaseWeight DECIMAL(6,3) NOT NULL,CategoryInteractionWeight DECIMAL(6,3) NOT NULL,VoiceConfirmedWeight DECIMAL(6,3) NOT NULL,
  IsActive BIT NOT NULL CONSTRAINT DF_PreferenceWeightVersions_IsActive DEFAULT(1),CreatedByUserId BIGINT NOT NULL,CreatedUtc DATETIME2(7) NOT NULL,
  CONSTRAINT FK_PreferenceWeightVersions_Tenants FOREIGN KEY(TenantId) REFERENCES dbo.Tenants(Id),CONSTRAINT FK_PreferenceWeightVersions_Users FOREIGN KEY(CreatedByUserId) REFERENCES dbo.Users(Id),
...
BEGIN
 CREATE TABLE dbo.CustomerPreferenceScores(
  Id BIGINT IDENTITY(1,1) NOT NULL CONSTRAINT PK_CustomerPreferenceScores PRIMARY KEY,TenantId BIGINT NOT NULL,CustomerId BIGINT NOT NULL,PreferenceType TINYINT NOT NULL,
  ReferenceId BIGINT NULL,Value NVARCHAR(200) NULL,Score DECIMAL(6,2) NOT NULL,WeightVersionId BIGINT NOT NULL,CalculatedUtc DATETIME2(7) NOT NULL,
  ...
  CONSTRAINT CK_CustomerPreferenceScores_Type CHECK(PreferenceType BETWEEN 1 AND 5),CONSTRAINT CK_CustomerPreferenceScores_Identity CHECK(ReferenceId IS NOT NULL OR NULLIF(LTRIM(RTRIM(Value)),N'') IS NOT NULL),CONSTRAINT CK_CustomerPreferenceScores_Value CHECK(Score BETWEEN 0 AND 100));
END;
GO
...
-- 10B: explicit Household-level tag. No VisitPartyId and no CCTV/face inferred source exists.
IF OBJECT_ID(N'dbo.HouseholdPreferenceTags',N'U') IS NULL
BEGIN
 CREATE TABLE dbo.HouseholdPreferenceTags(
  Id BIGINT IDENTITY(1,1) NOT NULL CONSTRAINT PK_HouseholdPreferenceTags PRIMARY KEY,TenantId BIGINT NOT NULL,HouseholdId BIGINT NOT NULL,PreferenceType TINYINT NOT NULL,ReferenceId BIGINT NULL,Value NVARCHAR(200) NOT NULL,
  Source TINYINT NOT NULL,CreatedByUserId BIGINT NOT NULL,Reason NVARCHAR(500) NULL,IsActive BIT NOT NULL CONSTRAINT DF_HouseholdPreferenceTags_IsActive DEFAULT(1),CreatedUtc DATETIME2(7) NOT NULL,UpdatedUtc DATETIME2(7) NOT NULL,
  CONSTRAINT FK_HouseholdPreferenceTags_Households_TenantHousehold FOREIGN KEY(TenantId,HouseholdId) REFERENCES dbo.Households(TenantId,Id),CONSTRAINT FK_HouseholdPreferenceTags_Users FOREIGN KEY(CreatedByUserId) REFERENCES dbo.Users(Id),
  CONSTRAINT CK_HouseholdPreferenceTags_Type CHECK(PreferenceType BETWEEN 1 AND 5),CONSTRAINT CK_HouseholdPreferenceTags_Source CHECK(Source BETWEEN 1 AND 3),CONSTRAINT CK_HouseholdPreferenceTags_Value CHECK(NULLIF(LTRIM(RTRIM(Value)),N'') IS NOT NULL));
END;
GO
...
-- 10D: extend Phase 5 voice master without replacing TriggerKeyword/Alias tables.
...
BEGIN
 CREATE TABLE dbo.StoreVoiceCommandRuntimeSettings(
  StoreId BIGINT NOT NULL CONSTRAINT PK_StoreVoiceCommandRuntimeSettings PRIMARY KEY,TenantId BIGINT NOT NULL,LanguageCode NVARCHAR(20) NOT NULL CONSTRAINT DF_StoreVoiceRuntime_Language DEFAULT(N'en-IN'),
  RequireConfirmation BIT NOT NULL CONSTRAINT DF_StoreVoiceRuntime_Confirmation DEFAULT(1),ListeningTimeoutSeconds INT NOT NULL CONSTRAINT DF_StoreVoiceRuntime_Timeout DEFAULT(15),MinimumRecognitionConfidence DECIMAL(6,2) NOT NULL CONSTRAINT DF_StoreVoiceRuntime_Confidence DEFAULT(70),
  CreatedUtc DATETIME2(7) NOT NULL CONSTRAINT DF_StoreVoiceRuntime_Created DEFAULT(SYSUTCDATETIME()),UpdatedUtc DATETIME2(7) NOT NULL CONSTRAINT DF_StoreVoiceRuntime_Updated DEFAULT(SYSUTCDATETIME()),
  CONSTRAINT FK_StoreVoiceRuntime_Stores_TenantStore FOREIGN KEY(TenantId,StoreId) REFERENCES dbo.Stores(TenantId,Id),CONSTRAINT CK_StoreVoiceRuntime_Timeout CHECK(ListeningTimeoutSeconds BETWEEN 3 AND 120),CONSTRAINT CK_StoreVoiceRuntime_Confidence CHECK(MinimumRecognitionConfidence BETWEEN 0 AND 100));
END;
GO
...
-- 10E: one voice command lifecycle; only Confirmed may produce a factual VoiceConfirmed signal.
IF OBJECT_ID(N'dbo.VoiceCommandSessions',N'U') IS NULL
BEGIN
 CREATE TABLE dbo.VoiceCommandSessions(
  Id BIGINT IDENTITY(1,1) NOT NULL CONSTRAINT PK_VoiceCommandSessions PRIMARY KEY,TenantId BIGINT NOT NULL,StoreId BIGINT NOT NULL,StaffUserId BIGINT NOT NULL,CustomerId BIGINT NOT NULL,MatchedTrigger NVARCHAR(100) NOT NULL,
  RecognizedText NVARCHAR(250) NULL,RecognitionConfidence DECIMAL(6,2) NULL,ProposedPreferenceType TINYINT NULL,ProposedReferenceId BIGINT NULL,ProposedValue NVARCHAR(200) NULL,ConfirmationRequired BIT NOT NULL,Status TINYINT NOT NULL,
  ExpiresUtc DATETIME2(7) NOT NULL,ResolvedUtc DATETIME2(7) NULL,CreatedUtc DATETIME2(7) NOT NULL,UpdatedUtc DATETIME2(7) NOT NULL,
  CONSTRAINT FK_VoiceCommandSessions_Stores_TenantStore FOREIGN KEY(TenantId,StoreId) REFERENCES dbo.Stores(TenantId,Id),CONSTRAINT FK_VoiceCommandSessions_Customers_TenantCustomer FOREIGN KEY(TenantId,CustomerId) REFERENCES dbo.Customers(TenantId,Id),CONSTRAINT FK_VoiceCommandSessions_Users FOREIGN KEY(StaffUserId) REFERENCES dbo.Users(Id),
  CONSTRAINT CK_VoiceCommandSessions_Status CHECK(Status BETWEEN 1 AND 5),CONSTRAINT CK_VoiceCommandSessions_Confidence CHECK(RecognitionConfidence IS NULL OR RecognitionConfidence BETWEEN 0 AND 100),CONSTRAINT CK_VoiceCommandSessions_Type CHECK(ProposedPreferenceType IS NULL OR ProposedPreferenceType BETWEEN 1 AND 5),CONSTRAINT CK_VoiceCommandSessions_Period CHECK(ExpiresUtc>CreatedUtc AND (ResolvedUtc IS NULL OR ResolvedUtc>=CreatedUtc)));
END;
GO
...
-- 10G: reuse stable Phase 5 permission names; insert only if an older installation is missing one.
DECLARE @Phase10Permissions TABLE(Name NVARCHAR(150),Description NVARCHAR(300));
INSERT INTO @Phase10Permissions VALUES
(N'Preferences.View',N'View customer and verified-household preferences.'),(N'Preferences.Manage',N'Manage explicit preferences and run recalculation.'),(N'VoiceCommands.Use',N'Use store-configured voice commands.'),(N'VoiceCommands.View',N'View store voice-command settings.'),(N'VoiceCommands.Configure',N'Configure store voice triggers, aliases and runtime controls.'),(N'VoiceCommands.Audit',N'View preference and voice-command audit history.');
...
BEGIN
 ...
 SELECT Id,StoreId,UserId,Action,EntityType,EntityId,BeforeJson,AfterJson,CorrelationId,CreatedUtc,COUNT_BIG(1) OVER() TotalCount FROM dbo.AuditLogs WHERE TenantId=@TenantId AND (Action LIKE N'CustomerPreference%' OR Action LIKE N'HouseholdPreference%' OR Action LIKE N'PreferenceWeight%' OR Action LIKE N'VoiceCommand%' OR Action LIKE N'StoreVoice%') AND (@AllowedStoreIdsCsv IS NULL OR StoreId IS NULL OR EXISTS(SELECT 1 FROM STRING_SPLIT(@AllowedStoreIdsCsv,N',') s WHERE TRY_CONVERT(BIGINT,s.value)=StoreId)) ORDER BY CreatedUtc DESC,Id DESC OFFSET (@PageNumber-1)*@PageSize ROWS FETCH NEXT @PageSize ROWS ONLY;
END;
GO

IF NOT EXISTS(SELECT 1 FROM dbo.DatabaseVersions WHERE VersionNumber=N'V1.9.0') INSERT INTO dbo.DatabaseVersions(VersionNumber,Description,AppliedUtc,AppliedBy) VALUES(N'V1.9.0',N'Phase 10 preferences, verified-household aggregation, dynamic voice confirmation and versioned recalculation',SYSUTCDATETIME(),SUSER_SNAME());
GO

-- Final manual-install verification. Fail loudly if any required object is absent.
IF (SELECT COUNT(*) FROM dbo.DatabaseVersions WHERE VersionNumber=N'V1.9.0')<>1 THROW 52190,'V1.9.0 must exist exactly once.',1;
IF OBJECT_ID(N'dbo.CustomerPreferenceSignals',N'U') IS NULL OR OBJECT_ID(N'dbo.CustomerPreferenceScores',N'U') IS NULL OR OBJECT_ID(N'dbo.HouseholdPreferenceTags',N'U') IS NULL OR OBJECT_ID(N'dbo.PreferenceWeightVersions',N'U') IS NULL OR OBJECT_ID(N'dbo.StoreVoiceCommandRuntimeSettings',N'U') IS NULL OR OBJECT_ID(N'dbo.VoiceCommandSessions',N'U') IS NULL THROW 52191,'One or more Phase 10 tables are missing.',1;
IF OBJECT_ID(N'dbo.CustomerPreference_Get',N'P') IS NULL OR OBJECT_ID(N'dbo.HouseholdPreference_Get',N'P') IS NULL OR OBJECT_ID(N'dbo.PreferenceWeight_GetActive',N'P') IS NULL OR OBJECT_ID(N'dbo.PreferenceAudit_Search',N'P') IS NULL THROW 52192,'One or more Phase 10 procedures are missing.',1;
IF OBJECT_DEFINITION(OBJECT_ID(N'dbo.HouseholdPreference_Get')) LIKE N'%VisitPart%' THROW 52193,'Household preference procedure must not depend on VisitParty/co-visit.',1;
DBCC CHECKCONSTRAINTS WITH ALL_CONSTRAINTS;
PRINT 'Phase 10 Preferences & Voice installation completed successfully.';
