/*
  Phase 9 — Platform Billing standalone/manual SQL Server 2022 installer.

  Purpose:
  - Run directly in SSMS / Azure Data Studio against an existing Phase 8 V1.7.0 database.
  - No PowerShell and no sqlcmd :r directives.
  - Safe to run repeatedly.
  - Creates/repairs Phase 9 tables, permissions, indexes, constraints and read procedures.
  - Platform Billing remains separate from Phase 8 Retail Billing.
*/
USE [CustSearch_AI];
SET NOCOUNT ON;
SET XACT_ABORT ON;
...
IF DB_NAME() <> N'CustSearch_AI'
    THROW 51900, 'Run Phase 9 against the CustSearch_AI database.', 1;

IF OBJECT_ID(N'dbo.DatabaseVersions', N'U') IS NULL
    THROW 51901, 'dbo.DatabaseVersions is missing. Install the Phase 8 baseline first.', 1;

IF NOT EXISTS (SELECT 1 FROM dbo.DatabaseVersions WHERE VersionNumber = N'V1.7.0')
    THROW 51902, 'Phase 9 requires validated Phase 8 database version V1.7.0.', 1;

IF OBJECT_ID(N'dbo.SubscriptionPlans', N'U') IS NULL
    THROW 51903, 'dbo.SubscriptionPlans is missing. Phase 8/platform tenancy baseline is incomplete.', 1;
IF OBJECT_ID(N'dbo.Tenants', N'U') IS NULL
    THROW 51904, 'dbo.Tenants is missing. Phase 8/platform tenancy baseline is incomplete.', 1;
IF OBJECT_ID(N'dbo.TenantSubscriptions', N'U') IS NULL
    THROW 51905, 'dbo.TenantSubscriptions is missing. Phase 8/platform tenancy baseline is incomplete.', 1;
IF OBJECT_ID(N'dbo.Permissions', N'U') IS NULL OR OBJECT_ID(N'dbo.Roles', N'U') IS NULL OR OBJECT_ID(N'dbo.RolePermissions', N'U') IS NULL
    THROW 51906, 'Permission/RBAC baseline tables are missing.', 1;
...
BEGIN TRY
    ...
    EXEC sys.sp_executesql N'
        ALTER TABLE dbo.SubscriptionPlans WITH CHECK
        ADD CONSTRAINT CK_SubscriptionPlans_Limits
        CHECK (
            MaxStores > 0 AND MaxUsers > 0 AND MaxStaff > 0 AND MaxCameras > 0
    ...
    IF OBJECT_ID(N'dbo.CK_SubscriptionPlans_TrialDisplay', N'C') IS NULL
        EXEC sys.sp_executesql N'
            ALTER TABLE dbo.SubscriptionPlans WITH CHECK
            ADD CONSTRAINT CK_SubscriptionPlans_TrialDisplay
            CHECK (TrialDays >= 0 AND DisplayOrder >= 0);';

    IF OBJECT_ID(N'dbo.CK_SubscriptionPlans_FeatureLimitsJson', N'C') IS NULL
        EXEC sys.sp_executesql N'
            ALTER TABLE dbo.SubscriptionPlans WITH CHECK
            ADD CONSTRAINT CK_SubscriptionPlans_FeatureLimitsJson
            CHECK (FeatureLimitsJson IS NULL OR ISJSON(FeatureLimitsJson) = 1);';

    IF NOT EXISTS (
        SELECT 1 FROM sys.indexes
        WHERE object_id = OBJECT_ID(N'dbo.SubscriptionPlans')
          AND name = N'IX_SubscriptionPlans_Display'
    )
    ...
    /* =========================================================
       9B — Effective tenant quotas and subscription periods
       ========================================================= */
    ...
    EXEC sys.sp_executesql N'
        ...
        ADD CONSTRAINT CK_Tenants_Quotas
        CHECK (MaxStores > 0 AND MaxUsers > 0 AND MaxStaff > 0 AND MaxCameras > 0);';
    ...
    EXEC sys.sp_executesql N'
        ...
        SET CurrentPeriodStartUtc = COALESCE(CurrentPeriodStartUtc, StartsUtc),
            CurrentPeriodEndUtc = COALESCE(CurrentPeriodEndUtc, EndsUtc),
            CancelAtPeriodEnd = CASE WHEN AutoRenew = 0 THEN 1 ELSE CancelAtPeriodEnd END;';

    IF OBJECT_ID(N'dbo.CK_TenantSubscriptions_CurrentPeriod', N'C') IS NULL
        EXEC sys.sp_executesql N'
            ALTER TABLE dbo.TenantSubscriptions WITH CHECK
            ADD CONSTRAINT CK_TenantSubscriptions_CurrentPeriod
            CHECK (
                CurrentPeriodEndUtc IS NULL
                OR (CurrentPeriodStartUtc IS NOT NULL AND CurrentPeriodEndUtc > CurrentPeriodStartUtc)
            );';

    /* =========================================================
       9C — Platform invoices (never RetailInvoices)
       ========================================================= */
    ...
    BEGIN
        ...
        (
            ...
            CONSTRAINT CK_PlatformInvoices_Amounts CHECK(
                Subtotal >= 0 AND DiscountAmount >= 0 AND TaxAmount >= 0
                AND Total >= 0 AND PaidAmount >= 0 AND PaidAmount <= Total
            ),
    ...
    BEGIN
        ...
        (
            ...
            CONSTRAINT CK_PlatformInvoiceItems_Amounts CHECK(
                Quantity > 0 AND Rate >= 0 AND DiscountAmount >= 0 AND TaxAmount >= 0
    ...
    /* =========================================================
       9D — Platform payments / callback idempotency storage
       ========================================================= */
    ...
    /* =========================================================
       9F — Permission repair + globally unique permission names
       ========================================================= */
    IF EXISTS (SELECT 1 FROM dbo.Permissions WHERE Scope = 2 AND Name = N'PlatformBilling.Subscriptions.View')
       AND NOT EXISTS (SELECT 1 FROM dbo.Permissions WHERE Name = N'TenantPlatformBilling.Subscriptions.View')
        UPDATE dbo.Permissions SET Name = N'TenantPlatformBilling.Subscriptions.View', Description = N'View this tenant CustSearch platform subscription.'
        WHERE Scope = 2 AND Name = N'PlatformBilling.Subscriptions.View';
    ...
    IF EXISTS (SELECT 1 FROM dbo.Permissions WHERE Scope = 2 AND Name = N'PlatformBilling.Invoices.View')
       AND NOT EXISTS (SELECT 1 FROM dbo.Permissions WHERE Name = N'TenantPlatformBilling.Invoices.View')
        UPDATE dbo.Permissions SET Name = N'TenantPlatformBilling.Invoices.View', Description = N'View this tenant CustSearch platform invoices.'
        WHERE Scope = 2 AND Name = N'PlatformBilling.Invoices.View';
    ...
    IF EXISTS (SELECT 1 FROM dbo.Permissions WHERE Scope = 2 AND Name = N'PlatformBilling.Payments.View')
       AND NOT EXISTS (SELECT 1 FROM dbo.Permissions WHERE Name = N'TenantPlatformBilling.Payments.View')
        UPDATE dbo.Permissions SET Name = N'TenantPlatformBilling.Payments.View', Description = N'View this tenant CustSearch platform payments.'
        WHERE Scope = 2 AND Name = N'PlatformBilling.Payments.View';

    DECLARE @PlatformPermissions TABLE(Name NVARCHAR(150), Description NVARCHAR(300));
    INSERT INTO @PlatformPermissions(Name, Description) VALUES
    ...
    SELECT 1, p.Name, p.Description, 1, SYSUTCDATETIME()
    FROM @PlatformPermissions p
    ...
    INSERT INTO @TenantPermissions(Name, Description) VALUES
    ...
    INSERT INTO dbo.Permissions(Scope, Name, Description, IsActive, CreatedUtc)
    SELECT 2, p.Name, p.Description, 1, SYSUTCDATETIME()
    FROM @TenantPermissions p
    WHERE NOT EXISTS (SELECT 1 FROM dbo.Permissions x WHERE x.Name = p.Name);
    ...
    WHERE r.Scope = 1 AND r.IsActive = 1
      AND (
        r.NormalizedName IN (N'PLATFORMSUPERADMIN', N'PLATFORMBILLINGADMIN')
        OR (
            r.NormalizedName IN (N'PLATFORMOPERATIONSADMIN', N'PLATFORMAUDITOR')
            AND p.Name IN (
                N'PlatformBilling.Plans.View',
                N'PlatformBilling.Subscriptions.View',
                N'PlatformBilling.Invoices.View',
                N'PlatformBilling.Payments.View'
            )
        )
      )
      AND NOT EXISTS (
    ...
    JOIN dbo.Permissions p ON p.Scope = 2 AND p.IsActive = 1
      AND p.Name IN (
          N'TenantPlatformBilling.Subscriptions.View',
          N'TenantPlatformBilling.Invoices.View',
          N'TenantPlatformBilling.Payments.View'
      )
    WHERE r.Scope = 2 AND r.IsActive = 1
      AND r.NormalizedName IN (N'TENANTADMIN', N'TENANTOWNER', N'SHOPOWNER', N'AUDITOR')
      AND NOT EXISTS (
          SELECT 1 FROM dbo.RolePermissions rp
          WHERE rp.RoleId = r.Id AND rp.PermissionId = p.Id
      );
    ...
    /* =========================================================
       Read-only Phase 9 stored procedures.
       TenantId is always supplied by trusted server context for tenant APIs.
       ========================================================= */
    EXEC sys.sp_executesql N'
...
BEGIN
    SET NOCOUNT ON;
    SELECT
        Id, PlanCode, PlanName AS [Name], Description,
        MonthlyPrice, AnnualPrice, Currency, TrialDays,
        MaxStores, MaxUsers, MaxStaff, MaxCameras,
        MaxMonthlyRecognitions, MaxMonthlyApiCalls,
        FeatureLimitsJson, IsActive, DisplayOrder,
...
BEGIN
    ...
    SELECT
        ...
        ts.SubscriptionPlanId AS PlanId,
        sp.PlanCode,
        sp.PlanName,
        ts.BillingCycle,
        ts.Status,
        ts.StartsUtc AS StartUtc,
        ts.TrialEndUtc,
        ts.CurrentPeriodStartUtc,
        ts.CurrentPeriodEndUtc,
        ts.CancelAtPeriodEnd,
        ts.CancelledUtc,
        t.MaxStores,
        t.MaxUsers,
        t.MaxStaff,
...
BEGIN
    ...
    WHERE x.PlatformInvoiceId = @PlatformInvoiceId
      AND (@TenantId IS NULL OR i.TenantId = @TenantId)
    ORDER BY x.Id;
END;';
...
BEGIN
    ...
    WHERE (@TenantId IS NULL OR p.TenantId = @TenantId)
      AND (@PlatformInvoiceId IS NULL OR p.PlatformInvoiceId = @PlatformInvoiceId)
...
BEGIN
    SET NOCOUNT ON;
    IF @TenantId <= 0 THROW 51970, ''TenantId must be positive.'', 1;

    SELECT TOP (1)
        t.Id AS TenantId,
        t.TenantCode,
        t.DisplayName AS TenantName,
        ts.Id AS TenantSubscriptionId,
        ts.SubscriptionPlanId AS PlanId,
        sp.PlanCode,
        sp.PlanName,
        ts.BillingCycle,
        ...
        ts.TrialEndUtc,
        ts.CurrentPeriodStartUtc,
        ts.CurrentPeriodEndUtc AS RenewalUtc,
        ts.CancelAtPeriodEnd,
        ts.CancelledUtc,
        t.MaxStores,
        t.MaxUsers,
        ...
        (
            SELECT TOP (1) pp.Status
            FROM dbo.PlatformPayments pp
            WHERE pp.TenantId = t.Id
            ORDER BY pp.PaymentUtc DESC, pp.Id DESC
        ) AS LatestPaymentStatus
    FROM dbo.Tenants t
    LEFT JOIN dbo.TenantSubscriptions ts ON ts.Id = (
        SELECT TOP (1) ts2.Id
        FROM dbo.TenantSubscriptions ts2
        WHERE ts2.TenantId = t.Id
        ORDER BY ts2.StartsUtc DESC, ts2.Id DESC
    )
...
END;';
    ...
    /* =========================================================
       Version ledger only after all Phase 9 objects compile.
       ========================================================= */
    IF NOT EXISTS (SELECT 1 FROM dbo.DatabaseVersions WHERE VersionNumber = N'V1.8.0')
        ...
        VALUES(
            N'V1.8.0',
            N'Phase 9 platform plans, authoritative tenant subscriptions, separate platform invoices/payments and billing read procedures',
            SYSUTCDATETIME(),
            SUSER_SNAME()
        );
    ...
    IF OBJECT_ID(N'dbo.PlatformInvoices', N'U') IS NULL
       ...
       OR OBJECT_ID(N'dbo.PlatformPayments', N'U') IS NULL
        THROW 51980, 'Phase 9 platform billing tables are missing.', 1;
    ...
    IF OBJECT_ID(N'dbo.PlatformBilling_Plan_List', N'P') IS NULL
       ...
       OR OBJECT_ID(N'dbo.TenantPlatformBilling_Summary_Get', N'P') IS NULL
        THROW 51981, 'Phase 9 stored procedures are missing.', 1;

    IF COL_LENGTH('dbo.SubscriptionPlans', 'MaxStaff') IS NULL
       OR COL_LENGTH('dbo.Tenants', 'MaxStaff') IS NULL
       OR COL_LENGTH('dbo.TenantSubscriptions', 'CurrentPeriodEndUtc') IS NULL
        THROW 51982, 'Phase 9 subscription/quota columns are missing.', 1;
    ...
    IF NOT EXISTS (SELECT 1 FROM dbo.Permissions WHERE Scope = 1 AND Name = N'PlatformBilling.Subscriptions.View')
       OR NOT EXISTS (SELECT 1 FROM dbo.Permissions WHERE Scope = 2 AND Name = N'TenantPlatformBilling.Subscriptions.View')
        THROW 51983, 'Phase 9 platform/tenant billing permission separation is missing.', 1;
    ...
    IF EXISTS (
        ...
        THROW 51984, 'PlatformInvoices must never reference RetailInvoices.', 1;

    IF (SELECT COUNT(*) FROM dbo.DatabaseVersions WHERE VersionNumber = N'V1.8.0') <> 1
        THROW 51985, 'V1.8.0 must exist exactly once.', 1;
...
PRINT 'Phase 9 Platform Billing installation completed successfully.';
...
SELECT name AS Phase9Procedure
FROM sys.procedures
WHERE name IN (
