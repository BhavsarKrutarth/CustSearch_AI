/*
==============================================================
CustSearch AI - Direct SQL Server Phase 5 Runner
File          : database/run-phase5.sql
Database      : CustSearch_AI
SQL Server    : 2022+
Purpose       : Plain T-SQL equivalent of database/run-phase5.ps1.
Usage         : Open this file in SSMS/Azure Data Studio and Execute.
Dependency    : Phase 4 / V1.3.0 must already be installed.
Safety        : Idempotent; does not drop/truncate Phase 5 objects.
==============================================================
*/
...
/* ------------------------------------------------------------
   PRECHECK - Phase 4 foundation must exist before Phase 5.
------------------------------------------------------------ */
IF OBJECT_ID(N'dbo.DatabaseVersions', N'U') IS NULL
    THROW 51000, 'DatabaseVersions is missing. Complete earlier database phases first.', 1;
IF NOT EXISTS (SELECT 1 FROM dbo.DatabaseVersions WHERE VersionNumber=N'V1.3.0')
    THROW 51001, 'Phase 4 V1.3.0 is not installed. Run Phase 4 first.', 1;
IF OBJECT_ID(N'dbo.Tenants',N'U') IS NULL OR OBJECT_ID(N'dbo.Users',N'U') IS NULL
    THROW 51002, 'Tenants/Users foundation is missing.', 1;
IF OBJECT_ID(N'dbo.Roles',N'U') IS NULL OR OBJECT_ID(N'dbo.Permissions',N'U') IS NULL
   OR OBJECT_ID(N'dbo.UserRoles',N'U') IS NULL OR OBJECT_ID(N'dbo.RolePermissions',N'U') IS NULL
    THROW 51003, 'Roles/Permissions authorization foundation is missing.', 1;
IF OBJECT_ID(N'dbo.AuditLogs',N'U') IS NULL
    THROW 51004, 'Phase 4 AuditLogs table is missing.', 1;
GO

-- ============================================================
-- PHASE 5 - TENANT USERS / STORES / STAFF
-- SUB-PHASE: 5A - Tenant Users / Roles / Permissions
-- SUB-PHASE: 5B - User-Store Assignments
-- SUB-PHASE: 5C - Store Master
-- SUB-PHASE: 5D - Staff / Shifts / Presence
-- SUB-PHASE: 5E - Product Categories
-- SUB-PHASE: 5F - Dynamic Voice Settings
-- SUB-PHASE: 5G - Dashboard / Search Procedures
-- SUB-PHASE: 5H - Version / Validation
-- VERSION: V1.4.0
-- ============================================================

/* Phase 5C - Store Master & Canonical Location. */
...
/* Phase 5B - Authoritative user-to-store assignments. */
...
/* Phase 5D - Staff profile linked to a tenant user. */
...
/* Phase 5D - Presence signals are operational context only. */
...
/* Phase 5E - Store/category taxonomy. */
...
/* Phase 5F - Dynamic voice trigger per store; Aasha Add is only the default/example. */
...
/* Phase 5C - Audit logs become store-aware after Stores exists. */
IF NOT EXISTS (SELECT 1 FROM sys.foreign_keys WHERE name=N'FK_AuditLogs_Stores_StoreId')
    ...
    ADD CONSTRAINT FK_AuditLogs_Stores_StoreId FOREIGN KEY(StoreId) REFERENCES dbo.Stores(Id);
GO

/* Phase 5A/5D/5E/5F - Stable permission catalog. */
...
/* Phase 5A - Default tenant roles and role permission materialization. */
...
BEGIN
    ...
    (N'TenantAdmin',N'Full tenant administration.'),
    (N'TenantOwner',N'Business owner with full tenant operations.'),
    (N'ShopOwner',N'Shop owner with full tenant operations.'),
    (N'StoreAdmin',N'Assigned-store administration.'),
    (N'StoreManager',N'Assigned-store staff and customer operations.'),
    (N'Manager',N'Day-to-day operational management.'),
    (N'SalesStaff',N'Assigned-store staff operations.'),
    (N'CRMStaff',N'Customer CRM operations.'),
    (N'BillingStaff',N'Billing operations.'),
    (N'CameraOperator',N'Camera and recognition operations.'),
    (N'IntegrationAdmin',N'Integration administration.'),
    (N'Auditor',N'Read-only audit operations.');

    INSERT dbo.Roles(TenantId,Scope,Name,NormalizedName,Description,IsSystem,IsActive,CreatedUtc)
    SELECT @TenantId,2,r.Name,UPPER(r.Name),r.Description,1,1,SYSUTCDATETIME()
    FROM @Roles r
    WHERE NOT EXISTS(
        SELECT 1 FROM dbo.Roles x
        WHERE x.TenantId=@TenantId AND x.NormalizedName=UPPER(r.Name)
    );
    ...
    WHERE r.TenantId=@TenantId
      AND r.NormalizedName=N'SALESSTAFF'
      AND p.Scope=2 AND p.IsActive=1
...
/* Apply default roles/permissions to all existing tenants. */
DECLARE @Phase5TenantId BIGINT;
DECLARE Phase5TenantCursor CURSOR LOCAL FAST_FORWARD FOR SELECT Id FROM dbo.Tenants;
OPEN Phase5TenantCursor;
FETCH NEXT FROM Phase5TenantCursor INTO @Phase5TenantId;
WHILE @@FETCH_STATUS=0
BEGIN
    EXEC dbo.Tenant_ProvisionDefaultRoles @TenantId=@Phase5TenantId;
    FETCH NEXT FROM Phase5TenantCursor INTO @Phase5TenantId;
END;
CLOSE Phase5TenantCursor;
DEALLOCATE Phase5TenantCursor;
GO

/* Phase 5G - Customer Admin dashboard summary. */
...
/* Phase 5C - Tenant-safe store search. */
...
/* Phase 5D - Tenant/store-safe staff search. */
...
BEGIN
    ...
    LEFT JOIN dbo.UserStoreAssignments usa
      ON usa.TenantId=s.TenantId AND usa.UserId=s.UserId
    WHERE s.TenantId=@TenantId
      AND (@StoreId IS NULL OR usa.StoreId=@StoreId)
...
/* ------------------------------------------------------------
   PHASE 5 VALIDATION - same intent as run-phase5.ps1 validation.
------------------------------------------------------------ */
IF (SELECT COUNT(*) FROM dbo.DatabaseVersions WHERE VersionNumber=N'V1.4.0') <> 1
    THROW 51050, 'Expected exactly one V1.4.0 Phase 5 database version row.', 1;
...
IF EXISTS(SELECT 1 FROM @MissingPhase5Tables)
    THROW 51051, 'One or more Phase 5 tables are missing.', 1;

IF OBJECT_ID(N'dbo.Tenant_ProvisionDefaultRoles',N'P') IS NULL
    THROW 51052, 'Tenant_ProvisionDefaultRoles is missing.', 1;
IF OBJECT_ID(N'dbo.TenantDashboard_GetSummary',N'P') IS NULL
    THROW 51053, 'TenantDashboard_GetSummary is missing.', 1;
IF OBJECT_ID(N'dbo.Store_Search',N'P') IS NULL
    THROW 51054, 'Store_Search is missing.', 1;
IF OBJECT_ID(N'dbo.Staff_Search',N'P') IS NULL
    THROW 51055, 'Staff_Search is missing.', 1;
...
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.Stores') AND name=N'UX_Stores_Tenant_StoreCode' AND is_unique=1)
    THROW 51056, 'UX_Stores_Tenant_StoreCode is missing.', 1;
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.UserStoreAssignments') AND name=N'IX_UserStoreAssignments_Tenant_Store')
    THROW 51057, 'IX_UserStoreAssignments_Tenant_Store is missing.', 1;
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.StaffProfiles') AND name=N'UX_StaffProfiles_Tenant_EmployeeCode' AND is_unique=1)
    THROW 51058, 'UX_StaffProfiles_Tenant_EmployeeCode is missing.', 1;
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.ProductCategories') AND name=N'UX_ProductCategories_Tenant_Store_Code' AND is_unique=1)
    THROW 51059, 'UX_ProductCategories_Tenant_Store_Code is missing.', 1;
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.StoreVoiceCommandAliases') AND name=N'UX_StoreVoiceCommandAliases_Tenant_Store_Alias' AND is_unique=1)
    THROW 51060, 'UX_StoreVoiceCommandAliases_Tenant_Store_Alias is missing.', 1;

IF NOT EXISTS(SELECT 1 FROM sys.foreign_keys WHERE name=N'FK_UserStoreAssignments_Stores_StoreId')
    THROW 51061, 'UserStoreAssignments -> Stores FK is missing.', 1;
IF NOT EXISTS(SELECT 1 FROM sys.foreign_keys WHERE name=N'FK_StaffShifts_Stores')
    THROW 51062, 'StaffShifts -> Stores FK is missing.', 1;
IF NOT EXISTS(SELECT 1 FROM sys.foreign_keys WHERE name=N'FK_StaffPresence_Stores')
    THROW 51063, 'StaffPresenceSessions -> Stores FK is missing.', 1;
IF NOT EXISTS(SELECT 1 FROM sys.foreign_keys WHERE name=N'FK_ProductCategories_Stores')
    THROW 51064, 'ProductCategories -> Stores FK is missing.', 1;
IF NOT EXISTS(SELECT 1 FROM sys.foreign_keys WHERE name=N'FK_StoreVoiceCommandSettings_Stores')
    THROW 51065, 'StoreVoiceCommandSettings -> Stores FK is missing.', 1;
IF NOT EXISTS(SELECT 1 FROM sys.foreign_keys WHERE name=N'FK_AuditLogs_Stores_StoreId')
    THROW 51066, 'AuditLogs -> Stores FK is missing.', 1;

DECLARE @MissingPhase5Permissions TABLE(Name nvarchar(150));
INSERT @MissingPhase5Permissions(Name)
SELECT required.Name
FROM (VALUES
    (N'TenantUsers.View'),(N'TenantUsers.Create'),(N'TenantUsers.Edit'),(N'TenantUsers.Deactivate'),(N'TenantUsers.AssignRoles'),
    (N'TenantStores.View'),(N'TenantStores.Create'),(N'TenantStores.Edit'),
    (N'Staff.View'),(N'Staff.Manage'),(N'StaffTracking.View'),
    (N'StoreCategories.View'),(N'StoreCategories.Manage'),
    (N'VoiceCommands.Use'),(N'VoiceCommands.View'),(N'VoiceCommands.Configure'),(N'VoiceCommands.Audit')
) required(Name)
WHERE NOT EXISTS(
...
IF EXISTS(SELECT 1 FROM @MissingPhase5Permissions)
    THROW 51067, 'One or more Phase 5 permissions are missing/inactive.', 1;
GO

PRINT 'Phase 5 SQL Server script completed and validated successfully.';

/* Useful verification result sets. */
SELECT VersionNumber,Description,AppliedUtc,AppliedBy
FROM dbo.DatabaseVersions
WHERE VersionNumber IN(N'V1.3.0',N'V1.4.0')
...
WHERE s.name=N'dbo'
  AND o.name IN(
    N'Stores',N'UserStoreAssignments',N'StaffProfiles',N'StaffShifts',N'StaffPresenceSessions',
    N'ProductCategories',N'StoreVoiceCommandSettings',N'StoreVoiceCommandAliases',
    N'Tenant_ProvisionDefaultRoles',N'TenantDashboard_GetSummary',N'Store_Search',N'Staff_Search'
  )
