/*
 CustSearch AI — Phase 8 standalone SQL Server runner
 Version: V1.7.0 — Products & Retail Billing

 Run directly in SQL Server Management Studio / Azure Data Studio.
 Prerequisite: CustSearch_AI database with Phase 7 V1.6.0 installed.
 This file is intentionally standalone: no PowerShell and no SQLCMD :r includes.
*/
USE [CustSearch_AI];
GO
SET XACT_ABORT ON;
...
IF OBJECT_ID(N'dbo.DatabaseVersions',N'U') IS NULL
    THROW 51300,'DatabaseVersions is missing. Install the canonical database through Phase 7 first.',1;
IF NOT EXISTS(SELECT 1 FROM dbo.DatabaseVersions WHERE VersionNumber=N'V1.6.0')
    THROW 51301,'Phase 7 V1.6.0 must be installed before Phase 8.',1;
GO
...
-- 8F Explicit auditable spend attribution; no inferred identity source is accepted.
...
-- Standalone-runner validation gates
IF (SELECT COUNT(*) FROM dbo.DatabaseVersions WHERE VersionNumber=N'V1.7.0')<>1
    THROW 51390,'V1.7.0 version ledger must exist exactly once.',1;
IF OBJECT_ID(N'dbo.Products',N'U') IS NULL OR OBJECT_ID(N'dbo.ProductStoreAvailabilities',N'U') IS NULL OR OBJECT_ID(N'dbo.RetailInvoices',N'U') IS NULL OR OBJECT_ID(N'dbo.RetailInvoiceItems',N'U') IS NULL OR OBJECT_ID(N'dbo.RetailInvoicePayments',N'U') IS NULL OR OBJECT_ID(N'dbo.RetailInvoiceParticipants',N'U') IS NULL OR OBJECT_ID(N'dbo.RetailInvoiceItemAttributions',N'U') IS NULL
    THROW 51391,'One or more Phase 8 tables are missing.',1;
IF OBJECT_ID(N'dbo.Product_Search',N'P') IS NULL OR OBJECT_ID(N'dbo.RetailInvoice_Search',N'P') IS NULL OR OBJECT_ID(N'dbo.RetailInvoice_GetDetail',N'P') IS NULL OR OBJECT_ID(N'dbo.CustomerPurchaseHistory_Get',N'P') IS NULL OR OBJECT_ID(N'dbo.HouseholdPurchaseSummary_Get',N'P') IS NULL OR OBJECT_ID(N'dbo.RetailSalesSummary_Get',N'P') IS NULL OR OBJECT_ID(N'dbo.RetailSalesByProduct_Get',N'P') IS NULL OR OBJECT_ID(N'dbo.RetailSalesByCategory_Get',N'P') IS NULL OR OBJECT_ID(N'dbo.RetailPaymentSummary_Get',N'P') IS NULL
    THROW 51392,'One or more Phase 8 stored procedures are missing.',1;
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.RetailInvoices') AND name=N'UX_RetailInvoices_Tenant_Store_Number')
    THROW 51393,'Retail invoice tenant/store number uniqueness index is missing.',1;
IF NOT EXISTS(SELECT 1 FROM sys.indexes WHERE object_id=OBJECT_ID(N'dbo.RetailInvoiceParticipants') AND name=N'UX_RetailParticipants_OnePayer')
    THROW 51394,'Single-payer filtered index is missing.',1;
IF NOT EXISTS(SELECT 1 FROM sys.foreign_keys WHERE name=N'FK_RetailInvoices_Customers_TenantCustomer') OR NOT EXISTS(SELECT 1 FROM sys.foreign_keys WHERE name=N'FK_RetailInvoices_Households_TenantHousehold') OR NOT EXISTS(SELECT 1 FROM sys.foreign_keys WHERE name=N'FK_RetailInvoices_Visits_TenantStoreVisit') OR NOT EXISTS(SELECT 1 FROM sys.foreign_keys WHERE name=N'FK_RetailInvoices_Parties_TenantStoreParty')
    THROW 51395,'Tenant/store-safe invoice foreign keys are incomplete.',1;
IF OBJECT_DEFINITION(OBJECT_ID(N'dbo.Product_Search')) NOT LIKE '%@AllowedStoreIdsCsv%' OR OBJECT_DEFINITION(OBJECT_ID(N'dbo.RetailInvoice_Search')) NOT LIKE '%@AllowedStoreIdsCsv%' OR OBJECT_DEFINITION(OBJECT_ID(N'dbo.RetailSalesSummary_Get')) NOT LIKE '%@AllowedStoreIdsCsv%'
    THROW 51396,'Required server-authorized store predicates are missing.',1;
GO

PRINT N'Phase 8 SQL Server script completed and validated successfully.';
SELECT VersionNumber,Description,AppliedUtc,AppliedBy FROM dbo.DatabaseVersions WHERE VersionNumber IN(N'V1.4.0',N'V1.5.0',N'V1.6.0',N'V1.7.0') ORDER BY VersionNumber;
SELECT name AS Phase8Table FROM sys.tables WHERE name IN(N'Products',N'ProductStoreAvailabilities',N'RetailInvoices',N'RetailInvoiceItems',N'RetailInvoicePayments',N'RetailInvoiceParticipants',N'RetailInvoiceItemAttributions') ORDER BY name;
SELECT name AS Phase8Procedure FROM sys.procedures WHERE name IN(N'Product_Search',N'RetailInvoice_Search',N'RetailInvoice_GetDetail',N'CustomerPurchaseHistory_Get',N'HouseholdPurchaseSummary_Get',N'RetailSalesSummary_Get',N'RetailSalesByProduct_Get',N'RetailSalesByCategory_Get',N'RetailPaymentSummary_Get') ORDER BY name;
GO
